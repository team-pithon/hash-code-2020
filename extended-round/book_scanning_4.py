...
def rank(book_list, ndays, rate):
    return (((score(book_list)-min_books_score) / (max_books_score - min_books_score)) + (1 - ((ndays - min_ndays)/(max_ndays - min_ndays))) + ((rate - min_rate)/(max_rate - min_rate))) * 100
...
with open(argv[1]+".txt", 'r') as f_in:
    ...
    for i in range(total_libraries):
        ...
        if s > max_books_score:
            max_books_score = s
        libraries.append(Library(i, nbooks, rate, ndays, book_list))

for lib in libraries:
    lib.rank = rank(book_list, ndays, rate)
